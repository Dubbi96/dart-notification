...
- **AOS Phase A2 진행 중** — #551 Strategy/Rule Versioning, #555 KRX 종가 후 `VersionActivation`, #557 `RiskPolicyVersion`, #559 `ApprovalRecord`·`ConfigAuditEvent` 불변 원장 기반까지 구현. 실제 정책·승인·역할 seed는 없고 AppModule/Cron/Signal/Risk/Order에는 미연결.
- **다음 A2 순서** — 승인 quorum·RBAC 정책 확정/버전화 → Strategy/Risk activation 결합 → Legacy constants DRAFT snapshot. 이후에만 A3 디바이스 실행 가능 공용 evaluator/Feature Snapshot dual-write로 이동.
...
---
**마지막 업데이트**: 2026-07-31 (AOS Phase A2 #559 Approval/ConfigAudit 기반·다음 순서 반영) / 이전: AOS #557
