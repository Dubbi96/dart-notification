 ...
 */
export type TablesOffloadCycleStatus = 'RAN' | 'SKIPPED' | 'WINDOW_SKIPPED';

/**
 * TablesOffloadScheduler (DAR-399) — 과거 tables 오프로드 마이그레이션 cron.
 *
 * ★카덴스: 매 10분 발화(env 오버라이드 유연성 유지). 단, DAR-503 로 실제 드레인은 **주말 창**에만
 *   수행한다 — tables LEFT JOIN 대형 스캔이 주중 DB 커넥션 풀을 독점(health 503)하던 것을 해소.
...
export class TablesOffloadScheduler {
  ...
  /** 매 10분 발화 — 주말 창에서만 과거 tables 오프로드 드레인(DAR-503). */
  @Cron('*/10 * * * *', { timeZone: KST_TIMEZONE })
  async drainOffload(now: Date = new Date()): Promise<TablesOffloadCycleStatus> {
    // DAR-503: 헤비 수집 창(기본 주말) 밖이면 드레인하지 않고 SKIPPED 로 기록(크론 생존 표면화).
    if (!isHeavyCollectionWindow(now)) {
      this.logger.log(
        'tables 오프로드 — 헤비 수집 창 밖(주중) 정지, 이번 사이클 WINDOW_SKIPPED(DAR-503)',
      );
      await this.recorder?.recordSkip(CRON_JOB_KEYS.TABLES_OFFLOAD_DRAIN);
