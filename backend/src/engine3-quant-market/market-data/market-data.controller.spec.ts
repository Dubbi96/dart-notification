...
describe('StockQuoteService.getMinuteCandles (DAR-352·DAR-377)', () => {
  ...
  /** 저장 행(findMany select 형태). volume 은 BigInt 컬럼이지만 Number() 변환 경로 검증 위해 number 사용. */
  const storedRows = [
    { time: '0901', openPrice: 100, highPrice: 102, lowPrice: 99, closePrice: 101, volume: 500 },
    { time: '0902', openPrice: 101, highPrice: 103, lowPrice: 100, closePrice: 102, volume: 600 },
  ];

  function makeService(opts: {
    kis?: Partial<KisApiService>;
    storedLatest?: { tradeDate: string } | null;
  ...
  it('당일 KIS 0행 + 저장 최근일 존재 → STORED 폴백(최근 거래일 서빙)', async () => {
    ...
    const { svc, findFirst } = makeService({
      kis: { isConfigured: true, fetchMinuteCandles },
      storedLatest: { tradeDate: '20260618' },
      storedRows,
    });
