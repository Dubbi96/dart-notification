...
export class StockMinutePriceCollector {
  ...
  /** 분봉 캔들을 StockMinutePrice 에 멱등 적재한다(createMany skipDuplicates). 반환=신규 삽입 행수. */
  private async persistCandles(
    ...
    const data: Prisma.StockMinutePriceCreateManyInput[] = [];
    for (const c of candles) {
      // KIS time 은 HHMMSS — 분 단위 저장(HHMM). 형식 불량은 스킵.
