...
export class StockQuoteService {
  ...
  async getMinuteCandles(
    ...
    if (!latest?.tradeDate) return empty();
    return this.servedStored(stockCode, latest.tradeDate, asOf);
  }

  /** StockMinutePrice 저장분(특정 거래일)을 KisMinuteCandle 형태로 시간 오름차순 서빙한다. */
