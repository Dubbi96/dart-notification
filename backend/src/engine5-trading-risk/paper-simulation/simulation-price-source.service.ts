 ...
 *
 * ★DAR-364(가격 기준 = 실시간 실가 구동): 운영 기본은 REAL_THEN_SYNTHETIC 이며 보유 포지션 평가의
 *   1순위는 KIS 실시간 실가(REALTIME)다 → 실 KRX 일봉(REAL) → 합성(SYNTHETIC) 순 폴백. 따라서
 *   '30일 트랙레코드'는 합성 전용 트랙이 아니라 실시간 실가 구동으로 재정의되며(과거 백테스트와 분리),
 *   합성은 실데이터·실시간이 모두 부재한 종목의 최후 폴백·레거시 검증(SYNTHETIC 전용 모드)으로만 남는다.
 *   latestPriceRow 가 정하는 이 가격을 손절/익절 평가(evaluateExits)와 화면 표시(status)가 동일하게 쓴다.
 *
