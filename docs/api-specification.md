...
```
  ...
  - `resultJson` — `status`로 판별하는 유니온:
    - `ANALYZED`: `{ status:'ANALYZED', eventType, cause, evidence[], eventLinkage:'STRONG'|'MODERATE'|'WEAK'|'UNCLEAR', caveat }`
