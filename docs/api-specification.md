...
### 1.5 카카오 OAuth 콜백 (브라우저용)

**Endpoint**: `GET /auth/kakao/callback?code={code}&state={state}`

모바일이 `WebBrowser.openAuthSessionAsync(authUrl, returnUrl)` 로 카카오 인증을 열면, 카카오는 이 콜백으로 `code` 와 `state` 를 전달한다. `state` 형식은 `${nonce}~${encodeURIComponent(returnUrl)}` 이며 `returnUrl` 은 앱 복귀 주소(Expo Go=`exp://.../--/kakao`, 빌드=`gongsion://kakao`)다.

**동작 (DAR-443)**:
- 토큰 교환 성공 시 결과를 `state` 키로 임시 저장하고 **HTTP 302 redirect** 로 `returnUrl?state={state}` 로 이동시킨다.
- 실패(예: redirect_uri mismatch) 시에도 **302 redirect** 로 `returnUrl?error={사유}` 로 이동시켜 실패 사유를 모바일에 표면화한다.
- **HTML interstitial(`<script>setTimeout(...custom-scheme...)`)은 사용하지 않는다**: 인앱 브라우저(ASWebAuthenticationSession/Chrome Custom Tabs)는 사용자 제스처 없는 custom-scheme 자동 이동을 차단해 `openAuthSessionAsync` 가 복귀를 감지하지 못한다. 302 는 OS 레벨에서 `returnUrl` 네비게이션을 가로채므로 브라우저가 자동 종료되고 `result.type==='success'` 로 복귀한다.

**Response**: `302 Found`, `Location: {returnUrl}?state={state}` (성공) 또는 `Location: {returnUrl}?error={사유}` (실패)

복귀 후 모바일은 `GET /auth/kakao/result?state={state}` 로 실제 토큰을 조회한다(폴링 폴백 포함).
