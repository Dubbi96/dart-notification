    ...
    "trades": [
      {
        ...
        "exitTs": "2026-06-22T10:32:00+09:00",    // ★DAR-435 entryTs 와 동일 timebase·★DAR-444 가드레일로 항상 entryTs 이후·장중(09:00~15:30 KST)·OPEN 이면 null
...
**DAR-444 청산 시각 가드레일 + 과거 데이터 백필**: 거래이력 카드에 00·01시 등 장외 시각 매도·진입>청산 역전이 표시되던 버그(restore 된 과거 36행 전건 + #383 미반영 구코드가 생성)를 해소한다. ① **가드레일(재발 봉인)** — `closePosition`/`forceCloseAll` 가 청산 ts 영속 직전 순수 함수 `sealScalpExitTimebase(entryTs, candidateExitTs, tradeDate)` 로 불변식을 봉인: `exitTs` 는 항상 **entryTs 이후 · 장중(KST 09:00~15:30)**. timebase 회귀(예: `new Date()` 진짜 UTC instant, minuteTimestamp 파싱 실패 폴백)로 장외/역전 시각이 들어오면 장중 경계 `[max(entryTs, 09:00), 15:30]` 로 clamp + `ERROR` 로그(`holdMinutes` 도 재계산, ≥0). ② **백필(1회성)** — 마이그레이션 `20260624020000_dar444_backfill_intraday_exit_ts`(데이터 전용·스키마 무변경): UTC-naive 로 저장된 과거 `exitTs` 에 `+9h` 를 더해 naive-KST 로 환산 후 동일 장중 경계로 clamp·`holdMinutes` 재계산. 멱등(보정 후 장외/역전 행 0 → 재실행 noop). 백필 후 `SELECT count(*) ... WHERE "exitTs"<"entryTs" OR EXTRACT(HOUR FROM "exitTs") NOT BETWEEN 9 AND 15` = 0(dev·prod). 실주문 0·AI 금지영역 불침범.
