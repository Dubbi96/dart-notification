...
model User {
  ...
  // DAR-563: 알림 탭 seen 마커. null = 미방문(전체 이력 신규 취급). NotificationHistory.isRead(행별
  // 읽음·하이라이트)와 분리 — 알림 뱃지(unreadCount 등)는 이 값 기준 sentAt 비교로 산출한다.
...
model NotificationSettings {
  ...
  // DAR-424: 라이브 페이퍼 체결 알림 토글. OFF면 인박스·푸시 모두 생략(과알림 방지).
  // DAR-550(오너결정·공개 코호트 가드): 신규 가입자 기본값 ON→★OFF(기존 row 무손실).
  tradePushEnabled  Boolean @default(false)
...
// ·PRICE_MOVE(관심종목 급변동, 갭분석 W7)·EDITION(일일 에디션 발행 푸시, DAR-523 — 'signal' 카테고리)
// NotificationHistory.type 으로 사용. 멱등 키 @@unique([userId, type, refId]).
// 알림 카테고리(4 버킷): 공시(disclosure)·신호(signal=…+PRICE_MOVE+EDITION)·체결(trade)·운영(system=RISK_ALERT+OPS_ALERT).
...
| tradePushEnabled | Boolean | 체결 알림 토글 (DAR-424) | default: false (DAR-550 신규 가입자 기본 OFF·기존 무손실) |
...
| userId | String | 사용자 ID | FK -> users.id |
| type | NotificationType | DISCLOSURE/SIGNAL/EXIT/THESIS_VIOLATED/TRADE_ENTRY/TRADE_EXIT/RISK_ALERT/OPS_ALERT/PRICE_MOVE/EDITION | default: DISCLOSURE |
...
**DAR-563: 뱃지(seen) vs 행 읽음(isRead) 이원화**:
- `isRead`/`readAt`은 그대로 행별 읽음(하이라이트) 전용 — `markAsRead`/`markAllAsRead`(개별·전체 읽음 API)만 갱신한다.
- 알림 탭 뱃지(`meta.unreadCount`/`unreadByType`/`unreadByCategory`)는 `isRead`가 아니라 `sentAt > User.notificationsLastSeenAt`(미방문 시 전체 이력) 기준으로 별도 산출한다.
- `POST /notifications/seen`이 `User.notificationsLastSeenAt`을 현재 시각으로 갱신 — 탭 방문(열람) 시 뱃지만 0이 되고, 행 미읽음 하이라이트는 그대로 유지된다.
...
| eventType | EventType | 이벤트 타입 enum — 우선 추출 7종(SUPPLY_CONTRACT 등) + 지분변동 3종(DAR-87) + 미모델 분류 확대 18종(DAR-346, OTHER 축소) + 미모델 분류 재확대 4종(DAR-338: REGULATORY_ADMIN_NOTICE·VALUE_UP_PLAN·EARNINGS_PREANNOUNCEMENT·VOLUNTARY_MANAGEMENT_DISCLOSURE) + OTHER |
...
## §42. TesterEvent (`tester_events`) — 테스터 코호트 계측 (DAR-516, Wave A/A6)

**목적**: Play 테스터 12인 코호트의 **로그인 후 인앱 행동**(에디션 오픈·카드 탭·푸시 오픈·통계 노출·waitlist CTA·iOS 게이트 설문)을 오픈율·재방문 산출용으로 적재. `FunnelEvent`(비인증 온보딩 퍼널)의 인증판 형제 — 이 이벤트들은 로그인 후 참여이므로 `userId`(인증) 키로 적재한다. FK 없는 forward-only 감사 이력. ★가산·계측층 전용 — engine5(매매/리스크)·실주문·킬스위치 무접촉(M10 무오염).

> ★**PII 무수집(수용기준 1)**: 아래 3필드만 저장한다 — 종목/카드 식별자·자유텍스트·기기정보 미저장. `event` 는 화이트리스트(SSOT: `dto/record-tester-event.dto.ts`↔모바일 `utils/testerEvents.ts`)만 허용(`IsIn` 검증).

| 컬럼 | 타입 | 설명 |
|---|---|---|
| id | String PK | cuid |
| userId | String | 인증 사용자 ID(내부 식별자 — 개인정보 아님). (인덱스) |
| event | String | 화이트리스트 8종: `edition_open`\|`card_tap`\|`push_open`\|`stats_section_view`\|`waitlist_cta`\|`survey_ios_shown`\|`survey_ios_answer_yes`\|`survey_ios_answer_no` |
| createdAt | DateTime | 서버 스탬프 ts(naive-UTC). KST 일 버킷은 조회 시 `AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Seoul'` 파생 |

**인덱스**: `(userId, createdAt)`(재방문=활동일 시계열) · `(event, createdAt)`(오픈율·이벤트별 카운트). (마이그레이션 `20260717120000_dar516_tester_event` — create-only). 오픈율·재방문 집계 SQL·API 계약 정본: `docs/analytics/tester-cohort-instrumentation.md`, `docs/api-specification.md §31.8`.
...
## §44–51. AOS Strategy/Risk Versioning, Approval & Activation — Issue #551, #555, #557, #559
...
| `ApprovalRecord` | `aos_approval_records` | Strategy/Risk/Activation 대상 hash와 evidence hash, 결정, actor role snapshot, 사유, correlation/idempotency를 보존하는 승인 원장 |
| `ConfigAuditEvent` | `aos_config_audit_events` | Strategy/Rule/Version/Risk/Activation의 생성·DRAFT 변경·삭제·상태 전이·활성화 기록. USER/SYSTEM actor와 before/after/event hash를 보존 |
...
- 승인·설정 감사 원장은 update/delete/truncate 불가이며 정정은 새 record/event로만 추가
- 승인·감사 idempotency key는 각각 unique이며 동일 flow+actor+대상(+action/hash)의 정확한 재기록도 복합 unique index로 차단
- 대상·근거·event hash는 64자 소문자 SHA-256 형식
- Config audit는 USER actor에만 user FK를 요구하고 SYSTEM actor에는 user FK를 금지하며, action별 before/after hash 형태를 강제
- 사용자 actor는 insert 시 실제 `User` 존재를 trigger로 검증한 뒤 불변 logical reference로 보존해 계정 삭제가 원장을 cascade/update하지 않게 함
- 승인자 수·role key 값·동일 actor 허용 정책은 schema/seed로 결정하지 않음
...
- `20260731040000_aos_approval_config_audit` — 승인·설정 변경 append-only 원장과 actor/hash/idempotency 기반

네 마이그레이션 모두 운영 반영은 별도 승인 전 실행하지 않는다. Issue #555 서비스는
`AppModule`·Cron·Signal·Order 경로에 자동 등록하지 않은 제어평면 토대다.
Issue #557은 실제 정책 값·seed·활성화 서비스 없이 저장 계약과 DB 안전장치만 추가한다.
Issue #559는 실제 승인·역할 seed나 상태 전이 배선 없이 불변 원장 계약만 추가한다.
...
## §53. AOS Decision Ledger — Issue #566

**목적**: `FeatureSnapshot + pinned StrategyVersion + pinned RiskPolicyVersion`을 공유 evaluator로 평가한 영수증과 Rule별 실행 trace를 재현 가능한 불변 원장으로 보존한다. 기존 `TradingSignal`은 계속 정본이며 새 원장은 기본 OFF dual-write로 parity만 관측한다.

| 모델 | 테이블 | 역할 |
|---|---|---|
| `MarketRegimeSnapshot` | `aos_market_regime_snapshots` | 시장 관측값·출처·품질·분류 결과와 canonical hash. 승인된 분류 Rule이 없으면 임의 threshold 대신 `UNCLASSIFIED` 기록 |
| `SignalDecision` | `aos_signal_decisions` | evaluator receipt, Feature/Strategy/Risk 참조, mode, score·차단사유, legacy score delta·parity 상태 |
| `RuleEvaluationTrace` | `aos_rule_evaluation_traces` | 실행 순서, Rule/implementation/parameter hash, 상태, 기여도, reason code와 facts |

세 테이블 모두 UPDATE·DELETE·TRUNCATE를 DB trigger로 차단한다. `decisionKey`와 `(Feature, Strategy, Risk, evaluator, receiptHash)` replay key로 재시도를 멱등 처리하며 decision과 trace는 단일 transaction으로 기록한다. Legacy 비교 baseline은 정상 상태 전이를 거쳐 `BACKTESTED`에서 멈추므로 승인·활성·주문 권한이 없다.

`AOS_DECISION_DUAL_WRITE_ENABLED=false`가 기본값이다. ON이어도 FeatureSnapshot 기록 성공 뒤에만 실행하며 오류와 parity 불일치는 기존 신호·알림 경로에 전파하지 않는다. 운영 DB migration과 플래그 활성화는 별도 승인 전 수행하지 않는다.

마이그레이션: `20260801010000_aos_decision_ledger`.

---

## §54. AOS Version-pinned Backtest — Issue #568

**목적**: Strategy/Risk/Dataset/Evaluator 버전을 고정한 PIT(point-in-time) 백테스트 결과를 재생 가능한 영수증으로 보존한다. 기존 `backtest_runs`·`backtest_trades` 실행기는 호환 어댑터로 재사용하지만, 완료 결과를 삭제·덮어쓰는 경로는 제거하고 AOS 원장에는 새 행만 추가한다.

| 모델 | 테이블 | 역할 |
|---|---|---|
| `AosBacktestRun` | `aos_backtest_runs` | 실행 타입, Strategy/Risk 버전, Dataset hash, evaluator, 비용·성과·민감도, acceptance 상태와 receipt hash |
| `AosBacktestWindow` | `aos_backtest_windows` | TRAIN·VALIDATION·TEST walk-forward 구간과 구간별 성과 |
| `AosBacktestTrade` | `aos_backtest_trades` | SignalDecision 참조, Rule/Regime/Event/Persona, 거래 비용과 MAE/MFE가 포함된 거래 원장 |
| `AosBacktestAttribution` | `aos_backtest_attributions` | Rule·Regime·Event·Persona 차원의 성과 귀속 |
| `AosBacktestAcceptanceCriterion` | `aos_backtest_acceptance_criteria` | 기준별 실제값·임계값·통과 여부와 evidence hash |

`VERSIONED_EVALUATOR` 실행은 모든 signal의 `SignalDecision` 참조와 TRAIN·VALIDATION·TEST 구간을 요구한다. 전환 중인 기존 PIT 실행은 `LEGACY_PIT_ADAPTER`로 명시해 혼동을 막는다. 수용 임계값은 코드 기본값으로 임의 결정하지 않으며 승인된 정책이 주입되지 않으면 `NOT_EVALUATED`로 기록한다. 정책이 있을 때만 거래 수, 비용 후 수익, MDD, profit factor, OOS 수익, 종목 집중도, 이웃 파라미터 안정성을 평가한다.

다섯 테이블은 UPDATE·DELETE·TRUNCATE를 DB trigger로 차단한다. `replayKey` 및 Strategy/Risk/Dataset/receipt 복합 identity로 재시도를 멱등 처리한다. 결과 통과가 StrategyVersion의 승인·활성화를 자동 수행하지 않으며, 운영 DB migration은 별도 승인 전 실행하지 않는다.

마이그레이션: `20260801020000_aos_versioned_backtest`.

---

## §55. AOS Canonical Paper/Shadow Execution — Issue #570

**목적**: 기존 `PaperTrade`와 M11 `OrderRequest`를 유지한 채, 승인된 SignalDecision부터 PortfolioProposal → RiskDecision → OrderPlan → Order → 다중 Fill → 거래일별 Reconciliation까지 재생 가능한 정본 원장을 병행한다. 실행 mode는 `SHADOW | PAPER`만 존재하며 LIVE·broker adapter는 이번 스키마와 서비스에 없다.

| 영역 | 모델 | 핵심 불변식 |
|---|---|---|
| 계좌 분리 | `AosTradingAccount`, `AosCapitalBucket` | `LONG_TERM`과 `SYSTEM_TRADING`을 분리하고 SPGI 50%·VTI 30%·시스템 20% 계획 weight를 별도 bucket으로 기록. 시스템 bucket 자동 보전은 DB CHECK로 금지 |
| 결정 | `AosPortfolioProposal`, `AosRiskDecision` | Signal/Strategy/Risk version과 자본 snapshot·위반 code·hash를 append-only 기록. 우선순위는 Kill Switch > Hard Risk > sizing |
| 주문 | `AosOrderPlan`, `AosOrder`, `AosOrderFill` | plan 없는 주문 금지, idempotency key, 유효기간, 2~20일 horizon, 부분체결, 결정적 paper order/fill id. Plan/Order 본문은 불변이고 허용 상태 전이만 가능 |
| 대조 | `AosReconciliationRun`, `AosReconciliationBreak` | 계좌·거래일별 PaperTrade와 canonical fill 대조, break 발생 시 OPS_ALERT. run은 불변이며 break 정정은 HumanIntervention 연결 후 1회만 가능 |
| 운영감사 | `AosHumanIntervention`, `AosKillSwitchEvent` | Hard Risk 우회 action 자체가 없고, 무엇을·왜·전후 상태·인증·scope/mode·복구정책을 hash와 함께 기록 |

`AOS_CANONICAL_PAPER_LEDGER_ENABLED=false`가 기본값이다. ON이어도 완전한 versioned RiskPolicy limits가 없는 legacy parity 신호에는 임계값을 추정하지 않고 기록을 건너뛴다. 기존 PaperTrade 경로는 계속 실행되며 canonical 원장 실패가 현재 모의매매를 중단하지 않는다. canonical 내부에서는 Risk가 `BLOCK`한 plan을 만들지 않고, stale plan·Kill Switch 발동·중복 fill을 fail-safe로 거부한다.

마이그레이션: `20260801030000_aos_canonical_paper_shadow`. 운영 DB 반영과 플래그 활성화는 별도 승인 전 실행하지 않는다.

---

**AOS 최신 갱신**: 2026-08-01 (A5 canonical Paper/Shadow·account separation·risk/order/fill/reconciliation·intervention/kill-switch 원장, Issue #570)

## §56. AOS Operator RBAC & Command Receipt — Issue #572

| 모델 | 테이블 | 역할과 주요 제약 |
|---|---|---|
| `AosOperatorMembership` | `aos_operator_memberships` | 사용자별 단일 역할과 ACTIVE/SUSPENDED/REVOKED 상태. 부여자와 사유 보존 |
| `AosStepUpGrant` | `aos_step_up_grants` | 원문 jti 대신 SHA-256 저장, scope·5분 만료·소비시각. null→consumedAt 한 번 외 UPDATE 금지 |
| `AosOperatorCommandReceipt` | `aos_operator_command_receipts` | 명령·대상·사유·request/result JSON·actor role snapshot·correlation·이중 hash를 append-only 저장 |

역할은 VIEWER/EDITOR/APPROVER/RISK_OFFICER/ADMIN으로 분리한다. Step-up grant와 command receipt는
DELETE/TRUNCATE가 불가능하고 receipt는 UPDATE도 불가능하다. grant 1건은 receipt 최대 1건과만
연결되며, correlationId 유니크로 중복 명령을 차단한다.

마이그레이션: `20260801040000_aos_operator_console_rbac`. 운영 DB 적용은 별도 승인 전 실행하지 않는다.

**AOS 최신 갱신**: 2026-08-01 (A6 Operator RBAC·single-use Step-up·append-only command receipt, Issue #572)
