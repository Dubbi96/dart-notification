...
## 6. paperclip 이슈 보드 — 현황 (2026-06-05, main `f1b960e`)

**DAR-1~17 전부 ✅ done. M3~M10 엔진 코드 + 비용 거버넌스 완료.** (origin보다 37커밋 앞섬, 미push) · 621 테스트 그린(tsc 0)

추가 완료(DAR-14~17): 라이브 통합검증(LLM 16/16·공시당 $0.00018, KRX는 401 키승인대기) · M10 PaperTrade 엔진(engine5 신규, 체결시뮬·포트폴리오) · AI 비용 거버넌스(집계·한도가드·지표 API). **5개 엔진(engine1~5) 코드 전부 존재.**

### 🔑 임계경로: KRX 키 승인 (사용자)
M10 졸업(30일 모의운용·실비용)·M11·M12는 KRX 시세 실데이터 필요 → quant 체인(시세→통계→매수→백테스트→모의) 실데이터 검증 불가. **openapi.krx.co.kr에서 키 "승인" 후 진행.** 승인 전엔 fixture 코드 소진 상태로 대기.

---
### (구) M3~M9 이력 (2026-06-05, main `b73c547`)
582 테스트 그린(tsc 0)
