...
### 3-4b. ★소비자 등급 임계값 재보정 (2026-06-26, F9)
**문제:** §3-4가 전략 진입 임계만 분포에 맞춰 재보정(30~50)하고, 소비자 대면 등급(`SIGNAL_GRADE_THRESHOLDS`)은 **80/60/30 그대로** 남겼다 → 실측(max 88·p95 33)상 STRONG 0.01%·BUY ~0.1%로 등급 변별력이 붕괴(가장 엄격한 전략 진입≥50보다 소비자 BUY≥60이 더 빡빡한 역전).

**해소:** a-priori frozen 상수를 **STRONG 50 / BUY 45 / WATCH 30** 으로 재보정(DAR-413 전략 사다리와 정렬). 런타임 퍼센타일은 §3-4 point-in-time 원칙대로 기각. **BUY=45는 DAR-322 보수성 경계(무의미 규모 SHARE_BUYBACK score 41) 위**에 둬 가짜 BUY 인플레이션을 막는다(40이었다면 41이 BUY로 격상). `scoreBandOf` 라벨은 상수에서 파생(재발 방지). 영속 grade는 forward-only → 기존 신호는 `POST /signals/generate` 재생성 전까지 구등급.

**정직성 주의:** §3 검증표상 이 점수대(BUY 45·STRONG 50 부근)는 누적수익 음수 구간이다 → '매수' 라벨은 표시일 뿐, execution(실주문) 활성화 근거가 아니다(엣지 게이트 별도).
