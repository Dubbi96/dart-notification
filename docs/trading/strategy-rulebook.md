...
- **★`auto-kill:<track>` 경보 발화 조건·의미·dedupe (DAR-531 문서화·하드닝)**:
  - **발화 조건**(`checkAutoKill` 3레그 OR, frozen `DEFAULT_AUTO_KILL_CONDITIONS`): ① 연속손실 ≥ 5회(트랙 청산 시계열) `CONSECUTIVE_LOSS`, ② 시장 급락(**DEFAULT 비활성** `marketDropPct=0` → 미기여), ③ 최근 24h `CronRunLog` FAILED ≥ 임계 `API_ERROR`. 하나라도 충족하면 `shouldKill=true` 권고.
  - **의미**: **실제 킬스위치 발동이 아니다**(SHADOW 계측). 수신자에게 "자동 킬 조건이 감지됐다(측정 모드·실발동 없음)"를 알리는 관측 경보 — OPS_ALERT(RISK_ALERT 아님), WARNING.
  - **dedupe(DAR-531)**: 장중 모니터가 매 10분 재평가하므로 과거엔 `shouldKill` 인 **매 사이클마다 OPS_ALERT 를 재발행**해 수신자에게 10분마다 경보가 갔다(2026-07-17 유령 진입 사고 정황). 이제 `evaluateAutoKillShadow` 는 멱등 기록의 **에스컬레이션 전환(신규 발동 생성 또는 `ALLOW→SHADOW_VIOLATION`) 에서만 1회 발행** → track+거래일 당 하루 1건(수신자 스팸 0·소비측 NotificationHistory dedupe 와 이중 방어). 영속 실패 시 알림 보류(오알림 방지).
...
- 근거: 측정 트랙의 매매 행동을 측정 기간 중 바꾸면 M10 졸업 지표의 시계열이 오염된다(측정 오염 = 판정 무효; `cc-live-readiness-diagnosis §2` TB-1/TB-2 교훈과 동형). 이 기간에는 관측·알림·문서·데이터층 변경만 허용한다(견고화 계획 `cc-trading-robustness-plan-2026-07-03.md §4 Wave 0`).
