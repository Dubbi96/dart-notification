...
| worktree 유령 tsc 에러 (소스와 무관한 TS2554 등) | `tsconfig.json`의 `incremental: true` 때문에 크래시난 이전 런이 남긴 stale `dist/tsconfig.tsbuildinfo`를 tsc가 신뢰 → 실제 소스와 다른 시그니처로 오진(예: 1-param 생성자에 "Expected 2 arguments") | 워크트리에서 원인 불명 tsc 에러가 소스와 불일치하면 `rm -rf dist` 후 재실행으로 캐시 배제부터 확인 (DAR-538에서 재현: 삭제 즉시 0 에러) | DAR-538 2026-07-17 |
...
| 개장 직후 전일 분봉이 당일로 오라벨 → 미래봉 유령 진입(M10 오염) | 당일 분봉 미형성 시점(개장 직후)에 KIS 분봉(inquire-time-itemchartprice)이 **전일 세션 전체**를 반환하는데, 수집기가 봉 자신의 영업일(`stck_bsop_date`)을 검증 안 하고 요청 tradeDate(오늘)로 라벨 → 09:00~15:30 봉이 '오늘 미래 시각'으로 둔갑, 스캘프 스캐너가 미래 충족봉 기반 09:02 진입(2026-07-17 prod, 5건). "환경시계 vs KIS 실데이터일" 함정 재발(DAR-414/444 가드의 커버리지 구멍) | ① 수집 거부: `stock-minute-price.collector.ts` `persistCandles` 가 `KisMinuteCandle.tradeDate`(=stck_bsop_date) ≠ 요청 거래일이면 적재 거부(`rejectedByDate` 정직 로그). ② 스캐너 미래봉 하드가드: `scanEntrySignals(…, nowMs)` 가 `ts > now(KST naive)` 봉 제외(engine5 가 `now+9h` 주입). ③ 경보 dedupe: `auto-kill:<track>` OPS_ALERT 에스컬레이션 전환 1회만 발행. 재현 스펙 TZ=UTC(적재 0·진입 0). Risk 하드룰 무변경(수집·스캐너·알림 계층 해결) | DAR-531 |
