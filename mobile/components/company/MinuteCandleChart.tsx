...
function shortTime(hhmmss: string): string {
  if (!/^\d{6}$/.test(hhmmss)) return hhmmss;
...
export function MinuteCandleChart({
  ...
  return (
    <View style={styles.wrap}>
      {/* ★정직 라벨(E7) — 사용자 언어, 실시간 시세(장중 실제 거래 시각) 기준 */}
      <View style={styles.honestyRow}>
        ...
        <Text style={[typo.small, { color: colors.textTertiary, marginLeft: spacing.xs, flex: 1 }]}>
          실시간 시세 기준 · 장중 실제 거래 시각
          {asOfClock(asOf ?? '') ? ` (서버 조회 ${asOfClock(asOf ?? '')})` : ''}
        </Text>
      </View>

      {/* 선택(또는 최신) 캔들 요약 — 색 단독 금지, 시간·OHLC 평문 병기 */}
      <View
        style={styles.summary}
        accessibilityRole="text"
        accessibilityLabel={`${shortTime(active.time)} 시가 ${won(active.open)}, 고가 ${won(
          active.high,
        )}, 저가 ${won(active.low)}, 종가 ${won(active.close)}, 거래량 ${active.volume.toLocaleString(
          'ko-KR',
        )}주`}
      >
        <Text style={[typo.captionMedium, { color: colors.text }]}>{shortTime(active.time)}</Text>
        ...
        {width > 0 ? (
          <View style={{ height: PRICE_HEIGHT }}>
            ...
            <View
              ...
              accessibilityLabel="가격 차트 — 좌우로 문질러 시점 선택"
              accessibilityValue={{ text: `${shortTime(active.time)} 종가 ${won(active.close)}` }}
              accessibilityActions={[
                { name: 'increment', label: '다음 시점' },
                { name: 'decrement', label: '이전 시점' },
              ]}
      ...
      <View style={styles.axisRow}>
        <Text style={[typo.small, { color: colors.textTertiary }]}>{shortTime(first.time)}</Text>
        <Text style={[typo.small, { color: colors.textTertiary }]}>
          시가 {won(dayOpen)} → 종가 {won(last.close)}
        </Text>
        <Text style={[typo.small, { color: colors.textTertiary }]}>{shortTime(last.time)}</Text>
      </View>

      {n >= 2 ? (
        <Text
          accessibilityRole="text"
          accessibilityLabel="차트를 좌우로 문지르면 해당 시각의 시·고·저·종과 거래량을 볼 수 있습니다."
          style={[typo.small, { color: colors.textTertiary, marginTop: spacing.xs }]}
        >
          차트를 좌우로 문지르면 해당 시각의 가격·거래량을 볼 수 있어요.
        </Text>
      ) : null}
    </View>
  );
