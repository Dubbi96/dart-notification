/**
 * DAR-227 결정론적 검증: 신호 화면 헤더(archiveHeader/sellHeader) 비메모이즈로
 * 타이핑(search 변경)마다 헤더 서브트리가 통째로 재생성 → 메타 배너·
 * SegmentedButtons까지 리렌더되던 성능 회귀.
 *
 * (DAR-535: 구 CurationSlot 헤더 조각은 신호탭 에디션 축 전환으로 폐기됐다. 이 가드는
 *  이제 search 무관 메모이즈 조각의 대표로 archiveHeader 내 metaBanners(deps [colors,typo])를
 *  모델링한다 — 계약 자체는 동일: search에 의존하지 않는 조각은 타이핑 중 리렌더 0.)
 *
 * React 재조정 모델:
 ...
 * - 따라서 search에 의존하지 않는 조각(metaBanners·feedToggle)은 타이핑 중
 *   참조가 불변 → 리렌더 0. search에 의존하는 searchInput만 리렌더.
 *
 * 본 스크립트는 index.tsx의 메모이즈 의존성 배열을 그대로 모델링해
 * "타이핑 시퀀스에서 metaBanners/SegmentedButtons 리렌더 0"을 증명한다.
...
// 안정 참조(컴포넌트 수명 내 불변): useMemo([colors,typo])·useCallback([])·useState setter.
const STABLE = {
  theme: 'theme:colors+typo', // metaBanners useMemo deps [colors, typo] — 타이핑/탭전환 중 불변
...
function metaBannersKey(): string {
  return `metaBanners:${STABLE.theme}`; // deps: [colors, typo] — search/feedTab 무관
}
...
function renderHeadersNaive(prev: boolean, _s: State): RenderResult {
  ...
  return { rerendered: prev ? ['metaBanners', 'searchInput', 'feedToggle'] : ['metaBanners', 'searchInput', 'feedToggle'] };
}
...
for (const next of typed) {
  ...
  if (r.result.rerendered.includes('metaBanners')) metaRerenders += 1;
...
assert('타이핑 중 metaBanners 리렌더 0', metaRerenders === 0);
...
// 3) 회귀 가드: 검색을 비워도 metaBanners/feedToggle 불변.
const cleared = renderHeaders(pass.keys, { search: '', feedTab: 'buy' });
assert('검색 클리어 시 metaBanners/feedToggle 리렌더 0', !cleared.result.rerendered.includes('metaBanners') && !cleared.result.rerendered.includes('feedToggle'));
assert('검색 클리어 시 SignalSearchInput만 리렌더', cleared.result.rerendered.length === 1 && cleared.result.rerendered[0] === 'searchInput');
pass = cleared;

// 4) 탭 전환(feedTab 변경): feedToggle만 리렌더, metaBanners 불변.
...
assert('탭 전환 시 metaBanners 리렌더 0', !switched.result.rerendered.includes('metaBanners'));

// 5) 음성 대조군: 메모이즈 미적용(수정 전)이면 타이핑마다 전 조각 리렌더(회귀 신호).
const naive = renderHeadersNaive(true, typed[0]);
assert('비메모이즈(수정 전): 타이핑 시 metaBanners·SegmentedButtons 리렌더 발생', naive.rerendered.includes('metaBanners') && naive.rerendered.includes('feedToggle'));
