...
const RISK_DAILY_LOSS_IMPLEMENTATION = ["risk", "daily-loss", "v1"].join(".");
...
test("입력 object와 rule 배열 순서가 달라도 canonical receipt가 byte-stable하다", () => {
  ...
  const registry = {
    ...
    [RISK_DAILY_LOSS_IMPLEMENTATION]: ({ rule }) => {
      calls.push(rule.ruleKey);
      return {
        status: "PASS",
        scoreDelta: 0,
  ...
  const riskRule = createRule({
    ...
    implementationKey: RISK_DAILY_LOSS_IMPLEMENTATION,
    category: "RISK",
    priority: 10,
    weight: 0,
...
test("Hard Risk 필수 feature가 없으면 ABSTAIN 설정이어도 실행하지 않고 차단한다", () => {
  ...
  const result = evaluateRules(
    ...
    {
      [RISK_DAILY_LOSS_IMPLEMENTATION]: () => {
        called = true;
        return {
          status: "PASS",
          scoreDelta: 0,
